...
export function hashOtp(email:string,code:string){return createHash('sha256').update(`${email}:${code}:${process.env.SESSION_SECRET||''}`).digest('hex');}
