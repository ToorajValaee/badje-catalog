...
export const ADMIN_EMAIL = 'admin@publio.ir';
function key() { return new TextEncoder().encode(env().SESSION_SECRET); }
export async function createSession(){const token=await new SignJWT({admin:true,email:ADMIN_EMAIL}).setProtectedHeader({alg:'HS256'}).setIssuedAt().setExpirationTime('12h').sign(key());(await cookies()).set(ADMIN_COOKIE,token,{httpOnly:true,secure:env().COOKIE_SECURE==='true',sameSite:'strict',path:'/',maxAge:60*60*12});}
export async function clearSession(){(await cookies()).delete(ADMIN_COOKIE);}
export async function isAdmin(){const token=(await cookies()).get(ADMIN_COOKIE)?.value;if(!token)return false;try{const {payload}=await jwtVerify(token,key());return payload.admin===true&&payload.email===ADMIN_EMAIL;}catch{return false;}}
